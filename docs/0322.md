...
在本教程中，我们将使用 Django 创建一个天气应用；这个应用将显示搜索到的城市的天气。这是一个简单的 Django 项目，帮助初学者理解 Django 的基本概念。我们还将使用天气API来获取数据。
...
我们创建了**index.html**文件。我们将从天气API获取数据，并将其渲染到模板中，但现在我们将创建视图并映射到网址。在视图中添加下面的函数。
...
#### 注意-该API将需要一些时间来激活。如果没有激活，请在几分钟后尝试。
...
正如我们在上面的快照中看到的，我们已经成功地使用 Django 框架创建了一个天气应用。到目前为止，我们已经了解了一些命令，创建虚拟环境，最重要的是了解了API。我们还讨论了调用 API 和获取不同天气字段的国家代码、湿度、温度等。
