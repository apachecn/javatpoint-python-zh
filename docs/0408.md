...
MayaVi 不是预装 Python 扩展的 Python 内置模块，因此，如果我们想在 Python 程序中使用和导入这个模块，我们必须在系统中安装这个模块。MayaVi 模块的安装非常简单，可以通过多种方法完成，包括通过 conda 方法、通过 Jupyter 笔记本方法、通过 pip 方法等等。但是在本教程中，我们将只使用 pip installer 方法来执行 MayaVi 模块的安装，并通过在终端Shell中使用以下 pip 命令来安装该模块:
