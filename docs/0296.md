...
Apache Kafka 是一个开源的流媒体平台，最初由 LinkedIn 设计。后来，它被移交给Apache基金会，并于 2011 年开源。
...
我们可以在不同的地方使用ApacheKafka。让我们考虑Kafka的一些用例，这些用例可以帮助我们理解它的用法:

1.  **活动监控:**我们可以用Kafka来监控活动。该活动可能属于物理传感器和设备或网站。生产者可以发布来自数据源的原始数据，这些数据可以在以后用于发现趋势和模式。
...
4.  **ETL:** Kafka提供了几乎实时流的特性；因此，我们可以根据需求开发一个 ETL。
5.  **数据库:**基于我们前面提到的事情，可以说Kafka也充当了数据库的角色。它不是一个典型的数据库，具有按需求查询数据的特性，但是 Kafka 可以将数据存储到我们需要的任何时间，而不会消耗它。
...
2.  **生产者:**生产者是负责将数据发布到Kafka系统中的应用。他们公布他们选择的主题的数据。
3.  **Consumer:** 有一些 Consumers 应用使用发布到主题中的消息。消费者获得其偏好主题的订阅并消费数据。
4.  **经纪人:**经纪人是Kafka的一个实例，负责消息交换。我们可以将Kafka作为集群或单机的一部分。
...
Python 编程语言中有各种各样的库可以使用Kafka。其中一些库描述如下:

| 南号码 | 图书馆 | 描述 |
| one | **Kafka-Python** | 这是一个由 Python 社区设计的开源库。 |
| Two | **pykaka** | 这个库由 Parsly 维护，它声称是一个 Pythonic API。但是，我们不能像Kafka-Python 那样在这个库中创建动态主题。 |
| three | **融合PythonKafka** | 这个库是由 Confluent 提供的，作为 librdkafka 的一个薄薄的包装。因此，它的性能优于上述两种。 |

## 安装依赖项

我们将在这个项目中使用Kafka-Python。因此，我们可以使用 pip 安装程序手动安装它，如下所示:
...
在下面的例子中，我们将创建一个生产者，产生从 1 到 500 的数字，并将它们发送给Kafka经纪人。稍后，消费者将从代理读取这些数据，并将它们保存在 MongoDB 集合中。

使用Kafka的好处之一是，万一一个消费者崩溃了，另一个或固定的消费者会继续阅读前一个离开的地方。这是一个很好的方法，可以确认所有数据都被输入到数据库中，没有丢失数据或重复数据。
...
在上面的代码片段中，我们已经导入了所需的库和模块。现在，让我们初始化一个新的Kafka制作人。请注意以下参数:
...
让我们创造Kafka式的消费者。我们将使用 **KafkaConsumer()** 函数进行这项工作；所以让我们仔细看看这个函数中使用的参数。
