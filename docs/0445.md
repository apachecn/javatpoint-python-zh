...
**我们必须在系统的命令提示符Shell中写入下面给出的 pip 命令，以便通过 pip 安装程序安装 instagramy 模块:**
...
一旦我们在终端Shell中写入了上面给出的命令，我们只需按下“enter”键，instagramy 模块的安装过程就会开始。正如我们将看到的，安装过程开始了；我们必须等待一段时间，直到所有的依赖项都成功安装。
