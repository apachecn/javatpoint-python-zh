...
到目前为止，我们已经理解了选项实例和 optionparser 构造器的工作和实现。现在，我们将使用 optparse 模块及其函数根据输入值在命令行 shell 中打印一个表。我们将在程序中使用 `for`循环来循环输入值，因此，使用 optparse 模块的功能打印命令外壳中的表。这将帮助我们更好地理解使用 optparse 模块解析命令行参数的工作和实际实现。请看下面的示例程序，其中我们在命令外壳中打印了表:
